...
const FavoritesPhone = ({phoneCard}) => {
    ...
    const handleAddToFavorite = () =>{
        ...
        if(!favoriteItems){
          addFavoritesArray.push(phoneCard)
          localStorage.setItem('favorites', JSON.stringify(addFavoritesArray))
          swal("Good job!", "You clicked the button!", "success");
        }
        else{
          const  isExist = favoriteItems.find((phoneCard) => phoneCard.id === id)

          if(!isExist){
            addFavoritesArray.push(...favoriteItems, phoneCard)
           localStorage.setItem('favorites', JSON.stringify(addFavoritesArray));
           swal("Good job!", "You clicked the button!", "success");
          }
          else{
